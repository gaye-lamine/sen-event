...
function normalizeEvent(raw: any): EventItem {
  ...
  return {
    ...
    location: raw.venue_name || raw.address || raw.city || 'Dakar',
    venue: raw.venue_name || raw.venue || 'Lieu à confirmer',
    city: raw.city || 'Dakar',
    date: raw.start_date || raw.date || '',
    time: raw.start_time || raw.time || '',
    rawDate: raw.start_date ? `${raw.start_date}T${raw.start_time || '00:00:00'}Z` : new Date().toISOString(),
    ...
    attendeesCount: raw.organizer?.events_count || 120,
    rating: 4.8,
    reviewsCount: 24,
    organizer: raw.organizer
      ? {
          name: raw.organizer.name || `${raw.organizer.first_name || ''} ${raw.organizer.last_name || ''}`.trim(),
          initials: raw.organizer.initials || 'NT',
          eventsCount: raw.organizer.events_count || 1,
          memberSince: raw.organizer.member_since || '2026',
          verified: Boolean(raw.organizer.is_verified),
        }
      : undefined,
    ticketTiers,
